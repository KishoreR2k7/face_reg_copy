...
const ManageCamerasPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await addCamera(newCameraName, newIpAddress);
        ...
        <div className="container mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-lg h-fit">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        <label htmlFor="cameraName" className="block text-sm font-medium text-gray-700">Camera Name</label>
                        <input type="text" id="cameraName" value={newCameraName} onChange={(e) => setNewCameraName(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" placeholder="e.g., DroidCam-Phone-3" />
                    </div>
                    <div className="mb-4">
                        <label htmlFor="ipAddress" className="block text-sm font-medium text-gray-700">IP Address / URL</label>
                        <input type="text" id="ipAddress" value={newIpAddress} onChange={(e) => setNewIpAddress(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" placeholder="e.g., http://192.168.1.100:4343/video" />
                    </div>
                    ...
                    <table className="w-full text-sm text-left text-gray-500">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50 sticky top-0">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3">IP Address / URL</th>
                            </tr>
                        </thead>
                        <tbody>
                            {cameras.map(camera => (
                                <tr key={camera.camera_id} className="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 font-medium text-gray-700">{camera.name}</td>
